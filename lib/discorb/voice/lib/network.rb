...
module Discorb::Voice
  ...
  class VoiceWS
    ...
    # @param client [Discorb::Client] The regular client to which this vWS is bound
    ...
    def initialize(channel, client, token, session, endpoint)
    ...
    def send_heartbeat
      ...
      @client.logger.debug("Sending voice heartbeat at #{millis}")
    ...
    def send_speaking(value)
      @client.logger.debug("Speaking: #{value}")
      @client.send({
        op: 5,
        d: {
          speaking: value,
    ...
    def websocket_open
      ...
      send_init(@channel.guild.id, @client.user.id, @session, @token)
    end

    # @!visibility private
    def websocket_message(msg)
      @client.logger.debug("Received VWS message! #{msg}")
    ...
    def connect
      ...
      @client.logger.debug("Started websocket initialization, now waiting for UDP discovery reply")

      # Now wait for opcode 2 and the resulting UDP reply packet
      ip, port = @udp.receive_discovery_reply
      @client.logger.debug("UDP discovery reply received! #{ip} #{port}")

      # Send UDP init packet with received UDP data
      send_udp_connection(ip, port, @udp_mode)

      @client.logger.debug("Waiting for op 4 now")
    ...
    def init_ws
      ...
      @client.logger.debug("Connecting VWS to host: #{host}")
      ...
      @client.logger.debug("VWS connected")
