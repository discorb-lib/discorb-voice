...
module Discorb::Voice
  ...
  #
  class VoiceClient
    # @return [Discorb::Channel] the current voice channel
    ...
    # @return [Discorb::Voice::Encoder] the encoder used to encode audio files into the format required by Discord.
    attr_reader :encoder
    ...
    def initialize(channel, client, token, session, endpoint)
      ...
      @ws = VoiceWS.new(channel, client, token, session, endpoint)
      @udp = @ws.udp
    ...
    def play(encoded_io)
      ...
      if encoded_io&.pid
        begin
          ...
          $stderr.puts "Error while killing ffmpeg process: #{e}"
        end
      end
    ...
    def play_internal
      ...
      loop do
        ...
        if @length.positive?
          ...
          $stderr.puts "Length is zero! This shouldn't happen!"
        end
      end
